...
const getHighlightedRegion = (name: any, data: string | any[]) => {
...
const getRegionHoverColor = (name: any, data: string | any[]) => {
	if (data.length > 0 || name) {
		for (let i = 0; i < data.length; i++) {
			const elm = data[i];
			if (name === elm.name) {
...
const getRegionValue = (name: any, data: string | any[]) => {
	if (data.length > 0 || name) {
		for (let i = 0; i < data.length; i++) {
			const elm = data[i];
			if (name === elm.name) {
...
const MapChart = ({ setTooltipContent, data, mapSource, mapType }: { setTooltipContent: any; data: any; mapSource: any; mapType: any }) => {
	return (
...
const Map = (props: { data: any; mapSource: any; mapType: any; }) => {
...
const renderDataList = (data: any[]) => {
	const list = data.map((elm: { name: string; color: string | (string & {}) | undefined; value: string | number | boolean | React.ReactElement<any, string | React.JSXElementConstructor<any>> | Iterable<React.ReactNode> | React.ReactPortal | Iterable<ReactI18NextChild> | null | undefined; }) => (
...
export const RegiondataWidget = (props: { data: any; mapSource: any; mapType: any; title: any; content: any; list: any; }) => {
